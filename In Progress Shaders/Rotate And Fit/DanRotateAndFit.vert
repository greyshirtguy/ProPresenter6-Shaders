...
void main()
{
	// Rotate an integer amount of quarter turns (take a rotation value from 0-3, round to nearest integer value and then mulitply by 90)
	...
	float sf = 1.0;
	if (mod(round(Rotation),2) == 1)
	{
		// The rotation is "sideways" - lets auto-scale (based on assumption of 16:9 output)
		sf = 0.55;
	...
	vec2 scaled = vec2(vsTexture.x/sf, vsTexture.y/sf);
	vec2 rotated = rotate(scaled, -90*round(Rotation)*3.14159/180.0, sf, xt, yt);
	fsTexture = rotated;
}
